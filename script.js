...
// ==========================================
const products = [
    { id: 1, name: "PlayStation 5 Slim", price: 3499.90, brand: "Sony", category: "Eletrônicos", img: "https://images.unsplash.com/photo-1606813907291-d86efa9b94db?auto=format&fit=crop&q=80&w=400" },
    { id: 2, name: "Galaxy Buds3 Pro", price: 999.90, brand: "Samsung", category: "Eletrônicos", img: "https://images.unsplash.com/photo-1606220588913-b3aacb4d2f46?auto=format&fit=crop&q=80&w=400" },
    { id: 3, name: "DJI Mini 4 Pro", price: 4999.90, brand: "DJI", category: "Eletrônicos", img: "https://images.unsplash.com/photo-1507582020474-9a35b7d455d9?auto=format&fit=crop&q=80&w=400" },
    { id: 4, name: "Nintendo Switch OLED", price: 2199.90, brand: "Nintendo", category: "Games", img: "https://images.unsplash.com/photo-1578303512597-81e6cc155b3e?auto=format&fit=crop&q=80&w=400" },
    { id: 5, name: "Mascara", price: 130.90, brand: "Kerastase", category: "Beleza", img: "https://tse4.mm.bing.net/th/id/OIP.zQEaKcL9M0-6xa4WesdH0wHaHa?rs=1&pid=ImgDetMain&o=7&rm=3" }
];
...
function displayProducts(items) {
    ...
    items.forEach(p => {
        grid.innerHTML += `
            <div class="bg-white rounded-xl md:rounded-2xl p-2.5 md:p-4 shadow-sm border border-gray-100 hover:shadow-xl transition duration-300 group flex flex-col h-full">
                <div class="relative bg-gray-50 rounded-lg md:rounded-xl mb-2 md:mb-3 aspect-square flex items-center justify-center overflow-hidden">
                    <img src="${p.img}" class="object-cover w-full h-full group-hover:scale-110 transition duration-700">
                </div>
                <div class="flex-grow flex flex-col">
                    <span class="text-[8px] md:text-[10px] font-bold w-fit uppercase bg-brand-yellow/10 text-brand-yellow px-1.5 py-0.5 rounded">${p.brand}</span>
                    <h3 class="font-semibold md:font-bold text-xs md:text-sm text-gray-800 mt-1.5 line-clamp-2 leading-tight">${p.name}</h3>
                    <div class="mt-auto pt-1.5">
                        <p class="font-black text-sm md:text-xl text-brand-darker leading-none">R$ ${p.price.toLocaleString('pt-BR', {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
                    </div>
                </div>
                <div class="mt-2 pt-2 border-t border-gray-100 space-y-1.5 md:space-y-2">
                    <button onclick="openInstallmentModal('${p.name}', ${p.price})" class="w-full text-[9px] md:text-xs font-semibold text-gray-600 flex items-center justify-center gap-1 hover:text-brand-yellow transition">
                        <i class="fa-solid fa-calculator"></i> <span class="hidden md:inline">Simular</span> Parcelas
                    </button>
                    <button onclick="addToCart('${p.name}', ${p.price}, '${p.img}')" class="w-full bg-brand-darker text-white py-1.5 md:py-2.5 rounded-md md:rounded-lg text-[10px] md:text-sm font-bold hover:bg-brand-yellow hover:text-brand-darker transition-all flex items-center justify-center gap-1.5 shadow-sm">
                        <i class="fa-solid fa-cart-plus"></i> <span class="hidden md:inline">Adicionar</span><span class="md:hidden">Comprar</span>
                    </button>
                </div>
            </div>`;
    });
